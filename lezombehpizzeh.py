...
pygame.mixer.music.load('gameassets/theme.mp3')
...
MAX_BAD_REVIEWS = 5
SURVIVE_SECS = 20  # 10 minute
WIN_TIME = FRAME_RATE * SURVIVE_SECS

REG_SPEED = 2
SLOW_SPEED = 1

GAME_WINDOW = display.set_mode(WINDOW_RES)
display.set_caption("PEPPA vs FREINIMIES!")

background_img = image.load("gameassets/lehouse.jpg")
background_surf = Surface.convert_alpha(background_img)
BACKGROUND = transform.scale(background_surf, WINDOW_RES)

pizza_img = image.load("gameassets/Peppa.png")
pizza_surf = Surface.convert_alpha(pizza_img)
VAMPIRE_PIZZA = transform.scale(pizza_surf, (WIDTH, HEIGHT))

garlic_img = image.load("gameassets/FLAME.png")
garlic_surf = Surface.convert_alpha(garlic_img)
GARLIC = transform.scale(garlic_surf, (WIDTH, HEIGHT))

cutter_img = image.load("gameassets/DINOLAUNCH.png")
cutter_surf = Surface.convert_alpha(cutter_img)
CUTTER = transform.scale(cutter_surf, (WIDTH, HEIGHT))

pepperoni_img = image.load("gameassets/STITCHY.png")
...
class Counters:
    ...
    def draw_bad_reviews(self, game_window):
        if self.bad_rev_rect:
            game_window.blit(
                BACKGROUND,
                (self.bad_rev_rect.x, self.bad_rev_rect.y),
                self.bad_rev_rect,
            )
    ...
    def draw_timer(self, game_window):
        if self.timer_rect:
            game_window.blit(
                BACKGROUND,
                (self.timer_rect.x, self.timer_rect.y),
                self.timer_rect,
            )
        timer_surf = self.display_font.render(
            str((WIN_TIME - self.loop_count) // FRAME_RATE),
            True,
            WHITE,
        )
...
class PlayTile(BackgroundTile):
    def set_trap(self, trap, counters):
        ...
        if trap and trap.cost <= counters.pizza_bucks:
            return trap
        else:
            return None
...
class ButtonTile(BackgroundTile):
    def set_trap(self, trap, counters):
        if self.trap.cost <= counters.pizza_bucks:
            return self.trap
        else:
            return None
    ...
    def draw_trap(self, game_window, trap_applicator):
        if trap_applicator.selected == self.trap:
            draw.rect(game_window, SELECTED, (self.rect.x, self.rect.y, WIDTH, HEIGHT), 5)
